...
class TransformerDecoder:
    ...
    def sample_next_token(
        ...
        model_inputs["input_ids"] = predicted_token #每次只输入一个单词，但是有之前保存的past_key_value
    ...
    def _generate_no_beam_search(self, model_inputs: Dict[str, torch.Tensor]):
        ...
        for timestep in range(start_position, self.max_steps):
            ...
            generated_token_sequences[current_batch_indices, timestep] = predicted_tokens.squeeze(1).cpu() # 给此刻所有不完整句子的timestep赋值
        ...
        if self.output_log_probs:
            ...
            for batch_idx in range(0, generated_token_sequences.shape[0], self.num_return_sequences):
                ...
                for return_idx in range(self.num_return_sequences):
                    ...
                    log_prob_sequence = log_prob_sequence[:results["tokens"][batch_idx][return_idx].shape[0]]#只计算从第一个单词到最后一个单词的概率
