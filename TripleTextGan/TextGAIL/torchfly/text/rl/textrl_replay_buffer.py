...
class TextRLReplayBuffer:
    ...
    def update_batch(self, states,experts,pasts, actions, action_log_probs, rewards,expert_rewards, normalize_reward=True):
        ...
        if normalize_reward:
            ...
            normalized_rewards = (rewards - self.reward_mean) / (self.reward_std + 1e-5) # 避免了除以方差
            normalized_rewards = np.clip(normalized_rewards, -2.0, 2.0)
        else:
            normalized_rewards = rewards

        self.buffer.extend(zip(states,experts,pasts, actions, action_log_probs, rewards, normalized_rewards,expert_rewards))

    def update(self, state,expert,past, action, action_log_prob, reward,expert_reward, normalize_reward=True):
        ...
        self.buffer.append((state,expert,past, action, action_log_prob, reward, normalized_reward,expert_reward))
