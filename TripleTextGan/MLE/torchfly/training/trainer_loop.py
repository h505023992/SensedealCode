...
class TrainerLoop:
    def __init__(
        ...
        if self.rank == 0:
            ...
            if valid_dataloader_fn:
                print("validation_dataloader is successfully loaded!")
            else:
                print("validation_dataloader is not loaded!")
            if test_dataloader_fn:
                print("test_dataloader is successfully loaded!")
        else:
            print("For rank!=0, validation_dataloader is not loaded!")
    ...
    def train(self):
        # Training begins
        self.callback_handler.fire_event(Events.TRAIN_BEGIN)#触发train begin事件
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for batch in pbar:
                # send to cuda device
                batch = move_to_device(batch, self.device)
                self.model.predict(batch)#在这里触发TEST_BEGIN？奇怪奇怪奇怪。1.10回来：错误。
